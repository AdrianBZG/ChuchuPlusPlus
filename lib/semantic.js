		...
		if(tree.main.type == "Call") {
			...
			tree.main.children.forEach(function(child) {
				...
				// START COMPOUND ASSIGN PART
					...
					let usedAssignID;
					// Save the value in the symbol table if not exists (Compound special case)
					if(currentSymbolTable.vars[child.left.value]) {
						arrayToReturn = { "status": "error", "data": "Used ID '" + child.left.value + "' is already defined. Line: " + child.left.location.start.line + ", Column: " + child.left.location.start.column};
						return arrayToReturn;
					} else {
