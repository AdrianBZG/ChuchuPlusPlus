...
let callbackAction = (tree, args) => {        
    ...
    tree.constants.forEach(function(c) {
        ...
        if (currentSymbolTable.vars[c[0]]) {
			//console.log(c[1]);
			arrayToReturn = { "status": "error", "data": "Constant " + c[0] + " already defined. Line: " + c[1].start.line + ", Column: " + c[1].start.column};
			return arrayToReturn;
        }
    ...
    tree.variables.forEach(function(v) {
        ...
        if (currentSymbolTable.vars[v[0]]) {			
			arrayToReturn = { "status": "error", "data": "Variable " + v[0] + " already defined. Line: " + v[1].start.line + ", Column: " + v[1].start.column};
			return arrayToReturn;
        }
